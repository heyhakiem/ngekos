...
@section('content')
<div id="Background" class="absolute top-0 w-full h-[430px] rounded-b-[75px] bg-[linear-gradient(180deg,#F2F9E6_0%,#D2EDE4_100%)]">
        </div>
        ...
        <form action="{{ route('check-booking.show') }}" method="POST"
            ...
            <div id="InputContainer" class="flex flex-col gap-[18px]">
                <div class="flex flex-col w-full gap-2">
                    ...
                    @error('code')
                        <p class="text-sm text-ngekos-orange">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex flex-col w-full gap-2">
                ...
                @if (session('error'))
                    <p class="text-center text-ngekos-orange">{{ session('error') }}</p>
                @endif
